...
//		1. 서버의 포트번호 정함
		...
		int port = 8500;
//		포트는 0~65535사이를 지정할 수 있다. 
//		(단,1023번이하는 이미 기본적으로 사용중인 경우가 많으니 피하는 게 좋다
...
//		5. 연결된 클라이언트와 입출력 스트림 생성(얻어도기)
		//client가 서버요청시 소켓을 전달함.
		//이 소켓에 입,출력 스트림이 담겨 있음.
...
//		7. 스트림을 통해 읽고 쓰기
		//서버 -> 클라이언트에 메세지 전송
		//2022년 08월 26일 15:30:21 [서버 접속에 성공했다]
		Date now = new Date(); //java.util.date : 자바에서 시간을 나타내는 객체
								//new Date()르 객체 생성 시 현재 시간이 저장되어 있다.
