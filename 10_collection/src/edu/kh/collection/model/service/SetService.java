...
public class SetService {
	 ...
	 * 
	 * - 순서를 유지하지 않음(인덱스 x)
	 * - 그래서 중복 데이터 저장을 못하게 한다.(null도 1개만 저장 가능함. 중복 안 돼)
	 * 
	 * 
	 * 
	 */
	...
	public void ex1() {
		...
		//remove(int index)인데 인덱스가 없자나
		//remove(Object value) 일치하는 값을 지우겠다고 해야됨. 근데 Object -> String임.
		//remove(String value) :set에 저장된 객체 중 value가 같은 객체 제거
		
		
		//근데 반환형이 boolean임.
		//제거되면 true, 안 되면 flase임.(조건식에 쓸 수 있음)
		set.remove("야놀자");
		
		System.out.println(set.toString());
		
		//Set에 저장된 요소 하나씩 얻어오기
		//근데 인덱스가 없음. 어케 얻어와? 방법2개
	...
	public void ex2() {
		 ...
		 * 		->이 때 중복 판단에 사용하는 메서드는,
		 * 		  Object.equals() 메서드의 오버라이딩된 메서드
		 * 		=>student클래스 가서 ..equals 오버라이딩...
		 * 
		 * 
		 * Hash라는 단어가 붙언 컬렉션이 중복을 판단하는 방법
		 * 
		 ...
		 * 
		 * 
		 * HashSet => equals(),hashCode() 둘다 오버라이딩 되어 있어야 함.
		 * 
		...
		set.add(new Student("홍홍홍",20,"서울시 종로구",'F',100));
		set.add(new Student("홍길동",15,"서울시 중구",'M',50));
	...
	public void ex3() {
		...
		//Wrapper Class
			...
			//컬렉션은 객체만 저장 가능(기본자료형은 안됨)
		
			//ran은 왜 됨? 컬렉션에 기본 자료형도 저장할 수 있는 방법이 있기 때문!
		...
		// -> Wrapper 클래스의 parseXXX() 을 이용하면 숫자로 변경 가능
		
		System.out.println(num + 100) ; //223이 나옴.		
		System.out.println(dNum + 100) ; //223.123이 나옴.		
		...
		Integer i1 = 10; //Integer에게 int를 대입한 거잖아. 
						//컴파일러가 Integer = new Interger(int) 이렇게 바꿔줌.
		
		
		
		
		//Auto UnBoxing(자동 포장 해제)
		int i2 = i1; //int = Integer를 대입함
