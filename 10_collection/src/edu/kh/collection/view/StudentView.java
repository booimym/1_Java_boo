...
public class StudentView {
		...
		do {
	         ...
	         try {
		         ...
		         System.out.println();
	         		...
	         		sc.nextLine();//입력버퍼에 남아있는 잘못된 문자열을 꺼내와 제거함.(무한루프 안빠지는...방법)
	         	}
	...
	public void selectAll() {
		...
		//service가 가지고 있는 studentlist를 가져와야 함
		List<Student> stdList = service.getStdList();
		
		//일반 for문
		for(int i = 0; i<stdList.size(); i++) {
			
			System.out.println(stdList.get(i)/*.toString()*/); //참조변수명만 쓰면 알아서 toString붙여서 그 안의 필드를 보여줌.
		...
		//향상된 for문
		// - 배열 또는 컬렉션의 모든 요소를
		// 처음부터 끝까지 순차 접근할 때 사용하는 for문입니다.
	...
	public void updateStudent() {
		...
		// index가 일치하는 학생 정보를 수정하면 "성공"
		// index가 일치하는 학생이 없다면 "실패(인덱스 불일치)"를 출력하겠다.
	...
	public void removeStudent() {
		...
		if (s == null) {
			...
			System.out.println(s.getName() + "학생의 정보가 제거됨ㅋ");
	...
	public void selectStudent1() {
		...
		List<Student> resultList = service.selectStudent1(name); // student로 타입이 제한된 리스트가 반환됨.
		
		//resultList에 결과가 있는지 없는지 검사
		if( resultList.isEmpty()) { //List.isEmpty() : 리스트에 저장된 요소가 없으면 true
									//resultList.size() == 0
	...
	/**
	 * 6. 이름으로 검색(포함)- 이름이 완전히 동일한 사람 말고, 성이 김씨힌 사람 같은 거
	 * 
	 */
	public void selectStudent2() {
		...
		List<Student> resultList = service.selectStudent2(name); // student로 타입이 제한된 리스트가 반환됨.
