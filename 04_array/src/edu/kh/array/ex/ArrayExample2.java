...
public class ArrayExample2 {
	...
	public void ex1() {
		...
		for(int row =0;row < arr.length;row++) {
			...
			for(int col = 0;col <arr[row].length;col++) { //arr[row].length를 쓰는 이유 : 가변배열? 
...
//		1. char 입력 받기
		System.out.print("검색할 앓파벳을 입력해주세요 :");
		
		
		Scanner sc = new Scanner(System.in);
//		char input = sc.next();
//		nextChar()는 존재하지 않음 -> Scanner는 char 자료형 입력을 지원하지 않는다.
		...
		boolean flag = true; // true면 검색결과 없음, false면 검색결과 존재한다.
	...
	public void ex3() {
		...
		for(int row = 0 ; row < arr.length -1 ; row++ ) {
				...
				for(int col = 0;col <arr[row].length -1 ;col++) { //arr[row].length를 쓰는 이유 : 가변배열? 
					...
					//각 행의 마지막 열
					arr[row][arr[row].length-1] += ran; //누적
					//각 열의 마지막 행
					arr[arr.length-1][col] += ran; //누적
					//3행-1 = 2번행
					
					//마지막 행, 마지막 열
					arr[arr[row].length-1][arr.length-1] += ran;
	...
	public void ex4() {
		...
		arr[0] = new int[3]; //3개짜리 배열을 만들고 arr[0]에 대입하겠다.
		arr[1] = new int[4];
		arr[2] = new int[5];
		arr[3] = new int[2];
		arr[4] = new int[1];
